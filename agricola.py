...
class AgricolaBoard:
    def __init__(self):
        ...
        self.additional_resources = ['stone_western', 'stone_eastern', 'grain', 'pig', 'sheep', 'cattle']
    ...
    def increment_resources(self):
        for resource in self.available_resources:
            ...
            print(Fore.GREEN + f"Added {increment_amount} {resource} to accumulation spot in round {self.round_number}." + Style.RESET_ALL)
    ...
    def display_accumulation_spots(self):
        ...
        for resource, amount in self.accumulation_spots.items():
            if resource in self.available_resources:
                print(Fore.RED + f"\t{resource.capitalize()}: {amount}" + Style.RESET_ALL)
    ...
    def prompt_new_resource(self):
        ...
        while True:
            choice = input("Enter the number of the resource to add (or 'done' if none): ").lower()
            if choice.startswith('d'):
                break
            elif choice.isdigit() and 1 <= int(choice) <= len(self.additional_resources):
    ...
    def take_resource(self):
        ...
        while True:
            choice = input("Enter the number of the resource to take (or 'done' if none): ").lower()
            if choice.startswith('d'):
                break
            elif choice.isdigit() and 1 <= int(choice) <= len(self.available_resources):
                selected_resource = self.available_resources[int(choice) - 1]
                self.accumulation_spots[selected_resource] = 0
                print(f"You took {selected_resource.capitalize()} and reset its counter to 0.")
    ...
    def simulate_round(self):
        ...
        input_text = input("Press 'Enter' to advance to the next round, 'take' to take a resource, or 'exit' to quit: ").lower()
